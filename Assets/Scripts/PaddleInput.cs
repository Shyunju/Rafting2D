...
namespace Rafting
{
    public class PaddleInput : MonoBehaviour
    {
        ...
        [SerializeField] float moveDistance = 0.5f; // 이동 거리 (엑스축 기준)
        [SerializeField] float rotateAngle = 3.0f;  // 회전 각도 (제트축) 야수면 위로 음수면 아래로
        [SerializeField] Animator[] _paddles;
        float duration = 1.0f;     // 애니메이션 재생 시간(초)
        private void Start()
        {
        ...
        IEnumerator MoveAndRotate(int dir)
        {
            ...
            float targetRot = startRot + dir * rotateAngle;
